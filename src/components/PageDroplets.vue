<template>
  <div class="app-page page-droplets">
    ...
    <div class="page-droplets-nav">
      <el-button
        class="droplets-btn droplets-btn--left"
        type="success"
        v-on:click="handleMainPage"
        icon="el-icon-arrow-left"
        round
        >{{ $t('Go Back') }}</el-button
      >
      <el-button
        class="droplets-btn droplets-btn--right"
        type="primary"
        :disabled="dropletLoading"
        round
        @click="handleUpdateDroplets"
      >
    ...
    <div>
      <el-alert
        class="m-top"
        v-if="dropletEmpty === true"
        :title="$t('The list of servers is empty')"
        type="info"
      />
      <el-row v-loading.lock="deleteLoading" v-else>
        <el-col
          class="box-card-col"
          :span="11"
          v-for="(droplet, index) in dropletList"
          :key="index"
          :offset="index % 2 ? 1 : 0"
        >
          <el-card
            class="box-card m-top"
            v-bind:style="{
              opacity: deletedDroplets.indexOf(droplet.id) === -1 ? 1 : 0.3,
            }"
          >
            <div slot="header" class="clearfix">
              <strong
                >{{ droplet.name }}, {{ droplet.image.distribution }}
                {{ droplet.image.name }}</strong
              >
            </div>
            <div class="text item">
              <h4>
                {{ $t('IP') }}:
                <strong v-if="droplet.networks.v4.length > 0">{{
                  droplet.networks.v4[0].ip_address
                }}</strong
                ><strong v-else>N/A</strong>
              </h4>
              <h4>
                {{ $t('Type') }}: <strong>{{ droplet.type || 'N/A' }}</strong>
              </h4>
              <h4>
                {{ $t('Status') }}: <strong>{{ droplet.status }}</strong>
              </h4>
              <h4>
                {{ $t('Region') }}: <strong>{{ droplet.region.name }}</strong>
              </h4>
              <h4>
                {{ $t('Created') }}:
                <strong>{{ formattedDaysAgo(droplet.created_at) }}</strong>
              </h4>
              <el-button
                class="delete-button"
                v-if="
                  droplet.locked === false && deletedDroplets.indexOf(droplet.id) === -1
                "
                @click="() => handleDelete(droplet)"
                type="text"
                >{{ $t('Delete') }}</el-button
              >
            </div>
      ...
      res
        .then(
          res => {
            ...
            this.$message({
              message: this.$root.$t('Server successfully removed'),
              type: 'success',
              showClose: true,
            })
            this.handleUpdateDroplets()
          ...
          res => {
            this.$message({
              message: this.$root.$t(
                'Deletion error. Please update the list and try again.',
              ),
              type: 'error',
              showClose: true,
            })
