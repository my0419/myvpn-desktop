  ...
  <div>
    ...
    >
      <el-tab-pane
        ...
        type="card"
        :name="provider.name"
        :disabled="apikey.length > 0 && activeProvider() !== provider.name"
      >
        <span slot="label">
          <img
            class="provider-choose-logo"
            :src="imgPath + provider.logo"
            v-if="provider.logo"
          />
          <span class="provider-choose-title" v-if="provider.name === 'custom'">{{
            $t(provider.title)
          }}</span>
        </span>
