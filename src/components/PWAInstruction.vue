<template>
  <div v-if="isMobile" class="pwa-instruction">
    <div class="pwa-instruction-content">
      <h2 class="pwa-instruction-header">
        {{ $t('Install the MyVPN app directly from your browser') }}
      </h2>
      <p v-if="manual.steps.length > 1" class="pwa-instruction-text">
        {{ `${$t('Step')} ${carouselIndex + 1}` }}
      </p>
      <el-carousel
        ref="carousel"
        trigger="click"
      ...
      <div v-if="manual.steps.length > 1" class="pwa-instruction-step-controls btn-group">
        <el-button
          ...
          size="mini"
          v-on:click="onPrevStep"
          type="info"
          >{{ $t('Prev Step') }}</el-button
        >
        <el-button
          class="btn-group-item"
          :disabled="manual.steps.length - 1 === carouselIndex"
          size="mini"
...
export default {
  ...
  data() {
    return {
      ...
      checked: false,
      neverShowAgain:
        JSON.parse(localStorageService.get('my_vpn_never_show_instructions')) || false,
    }
  },
  methods: {
    close() {
      ...
      document
        .querySelector('.pwa-instruction--visible')
