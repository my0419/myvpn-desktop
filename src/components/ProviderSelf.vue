<template>
  <div>
    ...
    <el-form class="provider-self-form">
      <el-form-item>
        <el-checkbox v-model="isPrivateKey">{{
          $t('Private Key Authorization')
        }}</el-checkbox>
      </el-form-item>
      ...
      <el-form-item v-else>
        <el-input
          ...
          type="textarea"
          :rows="2"
          :placeholder="$t('Paste in the server private key')"
        >
        </el-input>
      </el-form-item>
    </el-form>
  ...
  methods: {
    handleChangeIp() {
      ...
      if (!configured) {
        this.ip = ''
        this.$message({
          message: this.$root.$t('IP field is not filled in correctly'),
          type: 'error',
          showClose: true,
        })
      }
