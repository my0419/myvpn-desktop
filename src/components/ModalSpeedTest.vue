          ...
          <tr :key="index" v-for="(region, index) in regions">
            ...
            <td class="stats region_mean">
              <span
                class="color"
                v-if="calcMean(index)"
                v-bind:style="{ color: regionHighlight(calcMean(index)) }"
              >
                {{ calcMean(index) }} {{ $t('ms') }}
              </span>
            ...
            <td class="stats region_lowest">
              <span
                class="color"
                v-if="calcLowest(index)"
                v-bind:style="{ color: regionHighlight(calcLowest(index)) }"
                >{{ calcLowest(index) }} {{ $t('ms') }}</span
              >
              <i v-else class="el-icon-loading"></i>
            </td>
            <td class="stats region_highest">
              <span
                class="color"
                v-if="calcHighest(index)"
                v-bind:style="{ color: regionHighlight(calcHighest(index)) }"
                >{{ calcHighest(index) }} {{ $t('ms') }}</span
              >
              <i v-else class="el-icon-loading"></i>
            </td>
