...
export class ProviderHetznerCloud extends ProviderBase {
  constructor(config) {
    ...
    this.client = axios.create({
      ...
      headers: {
        ...
        'Accept-Language': navigator.language === 'ru' ? 'ru' : 'en',
      },
    })
  }

  async identifyAccess() {
    await this.client.get('/locations').then(
      r => r,
      err => {
        throw new Error(
  ...
  async regions() {
    let regions = await this.client.get('/locations').then(
      res => {
  ...
  async serverList() {
    let droplets = await this.client.get('/servers').then(
      res => {
  ...
  async deleteServer(id) {
    await this.client.delete(`/servers/${id}`).then(
      r => r,
      err => {
  ...
  async addSshKey(publicKey) {
    ...
    return await this.client.post('/ssh_keys', { name, public_key: publicKey }).then(
      res => {
  ...
  async deleteSshKey(id, dropletId) {
    await this.client.delete(`/ssh_keys/${id}`).then(
      r => {},
      res => {
  ...
  async createServer(sshKeyId, region, protocol, startupCommand) {
    ...
    const type = await this.client.get('/server_types').then(
      res => {
    ...
    let server = await this.client.post('/servers', params).then(
      res => {
  ...
  async checkServer(id) {
    ...
    while (locked) {
      ...
      server = await this.client.get(`/servers/${id}`).then(
        res => {
          return res.data.server
        },
        res => {
    ...
    return {
      ...
      slug: id,
      ipv4:
        server.public_net && server.public_net.ipv4 && server.public_net.ipv4.ip
