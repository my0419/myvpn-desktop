...
export class ProviderCryptoServers extends ProviderBase {
  ...
  async identifyAccess() {
    await this.client.get('/regions').then(
      r => r,
      err => {
        let errMessage = 'You entered an incorrect key. Authentication failed.'
        throw new Error(
          err.response.data
            ? err.response.data.error.replace('Invalid API key.', errMessage)
            : errMessage,
        )
      },
    )
  ...
  async serverList() {
    let droplets = await this.client.get('/droplet/list').then(
      res => {
  ...
  async deleteServer(id) {
    await this.client.get('/droplet/delete/' + id).then(
      r => r,
      err => {
  ...
  async addSshKey(publicKey) {
    ...
    return await this.client.post('/ssh/create', { name, key: publicKey }).then(
      res => {
  ...
  async deleteSshKey(id, dropletId) {
    await this.client.get(`/ssh/delete/${dropletId}/${id}`).then(
      r => {},
      res => {
  ...
  async checkServer(id) {
    ...
    while (locked) {
      ...
      droplet = await this.client.get(`/droplet/check/${id}`).then(
        res => {
          return res.data
        },
        res => {
    ...
    return {
      ...
      slug: id,
      ipv4:
        droplet.networks && droplet.networks.length > 0
          ? droplet.networks[0].ipAddress
          : null,
    }
  }
}
