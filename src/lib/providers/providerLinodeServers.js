...
import storeType from '@/store/modules/type'
...
export class ProviderLinodeServers extends ProviderBase {
  ...
  async identifyAccess() {
    await this.client.get('linode/instances').then(
      r => r,
      err => {
        throw new Error(
          err.response.data.errors[0].reason.replace(
            'Invalid OAuth Token',
            'You entered an incorrect key. Authentication failed.',
          ),
        )
      },
    )
  ...
  async regions() {
    let regions = await this.client.get('regions').then(
      res => {
        return res.data
      },
      res => {
    ...
    return regions.data
      .map(v => {
        ...
        return {
          ...
          slug: v.id,
          speedtest: regionInfo
            ? `http://speedtest.${regionInfo.speedtest}.linode.com/empty.php`
            : null,
          available: true,
        }
      })
  ...
  async serverList() {
    let droplets = await this.client.get('linode/instances').then(
      res => {
  ...
  async deleteServer(id) {
    ...
    await this.client.delete('linode/instances/' + id).then(
      r => r,
      err => {
  ...
  async createServer(sshKeyId, region, protocol, startupCommand) {
    ...
    const type = await this.client.get('linode/types').then(
      res => {
  ...
  async checkServer(id) {
    ...
    while (locked) {
      ...
      droplet = await this.client.get(`linode/instances/${id}`).then(
        res => {
          return res.data
        },
        res => {
  ...
  async deleteStackScript() {
    if (this.stackScriptId !== null) {
      this.client
        .delete(`linode/stackscripts/${this.stackScriptId}`)
        .catch(err => console.log('failed delete stack script', err))
      this.stackScriptId = null
    }
  }
}
